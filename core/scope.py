...
whose four coordinates are themselves lattices:
  ...
  * **Σ (`StratumScope`)** — a downward-closed subset of the stratum-refinement forest R. Base
    ...
    ungrantable.
  * **E (`EdgeScope`)** — the edge-class fibers `E ⊆ {F, D}` (F = citation/warrant, D = supersession).
...
class StratumScope:
    """A downward-closed subset of R (a downset). Construct via `of(...)` (auto-closes); `meet`/`join`
    ...
    def of(cls, *strata: Stratum) -> StratumScope:
        ...
        here only to construct the denylist ideal itself — a real grant never names it (`admissible`)."""
        return cls(strata=_downward_close(strata))

    @classmethod
    def top(cls) -> StratumScope:
        """`⊤_Σ = R ∖ 𝔇` — the fullest grant: every base stratum + its refinements, minus FOUNDATION."""
...
class Clock(Enum):
    """A monotone coarsening of the ledger's causal order (the op-seq spine). The poset is ordered by
...
def common_refinement(a: Clock, b: Clock) -> Optional[Clock]:
    """The materialized clock ⪰-above BOTH `a` and `b` (their least common refinement), or None when
    the only such clock is parked (canonically N). None ⇒ a cross-clock T-meet is a constructor error.

    v1 poset: comparable clocks return the finer of the two; every other distinct pair resolves only
    through the parked N, hence None. Extensible at CS-a (materialize N ⇒ far fewer None results)."""
...
class Window:
    ...
    def __le__(self, other: Window) -> bool:
        ...
        if self == other:
            return True
        if self.kind is not WindowKind.ALL and other.kind is not WindowKind.EMPTY and self._orderable(other):
...
class TimeScope:
    ...
    def meet(self, other: TimeScope) -> TimeScope:
        ...
        raise NoCommonClockError(
            f"cross-clock window transport {self.clock.value!r} ⊓ {other.clock.value!r} is retrieval "
...
class WorldReach(IntEnum):
    ...
    → WorldReach` bridge lives ops-side (`ops.effects.world_reach`) so this module stays pure-core."""
...
class Scope:
    ...
    (grantable only by a holder of the join); ⊑ = "no more authority than". A cross-clock meet raises
    `NoCommonClockError`; a cut-less multi-stratum point scope raises `SliceError` at construction."""
    ...
    cut: Optional[Hashable] = field(default=None, compare=False)
    ...
    def __post_init__(self) -> None:
        if (len(self.sigma.strata) > 1
            ...
            raise SliceError(
                f"a multi-stratum scope (|Σ|={len(self.sigma.strata)}) with a point window on clock "
                f"{self.time.clock.value!r} needs an explicit consistent cut — bare 'now' is well-typed "
    ...
    def __le__(self, other: Scope) -> bool:
        """`⊑` on the four lattice coordinates ONLY (tier is an annotation, not a lattice element)."""
...
@dataclass(frozen=True)
class Ideal:
    """A firewall as an order-ideal over Σ: a grant is admissible for a client class iff it meets the
    ideal at ⊥ (`s.sigma ∩ ι = ∅`). Firewalls are subtracted from the space of scopes, not re-checked
...
def admissible(s: Scope, ideals: Iterable[Ideal]) -> bool:
    """`s` is admissible iff it meets every applicable firewall ideal at ⊥ (§2.2). The mirror-payload
