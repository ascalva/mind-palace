...
## Current phase
> Phase: 6 COMPLETE (2026-06-25) — verified against gate (129 passed: 123 logic + 6 live; 5 podman skipped, machine off). Interface layer (first Zone B): `core/interface.py` `CoreInbox` (reads/writes the handoff dir only — no network, no `import edge`) ↔ `edge/interface/` gateway+adapters over a **filesystem handoff, never imports** (Inv 2). `LocalAdapter` private default (Tailscale; `transits_third_party=False`); `WhatsAppAdapter` opt-in stub (Inv 11); gateway refuses third-party unless `allow_third_party`. `[interface]` config. ⚠️ Phase 4 empirical `-m podman` still pending — podman machine won't boot here; see `docs/runbook.md` → "Sandbox runtime". Next: Phase 7 (Curator + dreaming); Phase 6b voice is optional (§20.11, needs owner go-ahead). Update this line as phases complete.
```
