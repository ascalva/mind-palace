...
- **DRY — reuse before you re-implement.** A duplicated *implementation* is a **defect**, not a nit: two copies of the same logic drift, and the drift is a bug waiting to happen (bp-060 re-derived a Laplacian `core/complex` already owned — finding-0101). Before writing an algorithm or primitive, search for an existing one — **`core/` first** — and call it; extend or refactor the shared version rather than paste a second. This is enforced by process and review (the build-plan §2 manifest-audit step, finding-0103), not by a hook. See §Trust boundaries in code for the companion rule: core is self-contained *and* never re-implements what it already has.
...
- **Core is SACRED and self-contained.** Its only dependencies are the Python **stdlib** and **pinned, side-effect-free third-party libraries** (numpy, scipy, …; `uv.lock` authoritative — audited-once external contracts). It imports **NOTHING first-party outside `core/`**: config, eval, ops, agents, edge, and scheduler are all **machinery** built *around* core to test, study, calibrate, run, and enforce it, and first-party code core reaches for is a **liability** — mutable, side-effecting, coupling the sacred to the machinery's churn (the opposite of a pinned lib). No wiggle room, config included. The arrow is **`everything → core`**, never the reverse; and core never re-implements what it already has (the DRY rule, §Language & style). Template: **core computes and returns pure data; the machinery calls core, records, grades, runs.** Enforced structurally by `tests/unit/test_core_self_containment.py` (a dynamic import scan, red until the cleanup program lands — finding-0103).
