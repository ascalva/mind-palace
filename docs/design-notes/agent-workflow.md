...
**Clarification on (c) — diff against `HEAD`, not the session baseline (warrant: finding-0003).** The blessing detector compares the working tree to `HEAD`, flagging only an _uncommitted_ `→ ratified` or `proposed → ready` flip. The reason is that a committed blessing is already accountable — attributable to its commit author, which is exactly §10's "deliberate, logged." Diffing against the session baseline (HEAD-at-start) instead made a blessing _committed mid-session_ keep firing until the baseline was manually re-anchored — a spurious block on an already-accountable act. Anchoring to `HEAD` makes a committed blessing self-clear with no re-anchoring, while still catching an in-flight flip that has not yet been recorded. `session-baseline` is retained only for the SessionStart brief's "what changed this session" narration; enforcement does not depend on it.
