...
- `docs/findings/finding-0094.md` (spec-fidelity, RESOLVED in-plan): the commit clock's tick is
...
- [x] finding-0094 filed (spec-fidelity, builder-resolved)
...
4. The commit-as-range REAL check here uses a `version_seq`-grouped commit-ANALOGUE map (NOT git
   commits). A git-history-sourced commit map (finding-0094) is a separate ops-side follow-up; the
   live "against the actual git history" convexity claim is NOT yet mechanically verified — it is
   the finding-0094 re-entry, not a bp-053 deliverable.
